...
public class ProtocolTexture : ProtocolBase
{
    ...
    public void GetTex() {
        ...
        print("start "+start);
        Array.Copy(bytes, start+sizeof(Int32), texBytes, 0, bytes.Length - start-sizeof(Int32));
        print("得到的图片大小 "+texBytes.Length);
        string str1 = "";
        for (int i = 0; i < bytes.Length - start; i++) {
            int t = (int)texBytes[i];
            str1 += t.ToString() + " ";
        }
        print("收到的图片 " + str1);
    }
